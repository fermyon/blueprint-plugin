...
func showAllComponents(tomlData *SpinTOML, envVars map[string]string) string {
	...
	variableTable.AppendHeader(table.Row{"env_key", "env_value", "is_secret"})
	for varKey, varData := range tomlData.Variables {
		var matchExists bool
		for envKey, envValue := range envVars {
			if varKey == envKey {
				matchExists = true
				variableTable.AppendRow(table.Row{envKey, envValue, varData.Secret})
			}
		}

		if !matchExists {
			if varData.Required {
				variableTable.AppendRow(table.Row{varKey, "ERR: MISSING REQUIRED VALUE", "n/a"})
			} else if varData.Default == "" {
				variableTable.AppendRow(table.Row{varKey, "ERR: MISSING DEFAULT VALUE", "n/a"})
			} else {
				variableTable.AppendRow(table.Row{varKey, varData.Default, varData.Secret})
			}
		}
	}
...
func showSpecificComponent(tomlData *SpinTOML, envVars map[string]string, componentName string) (string, error) {
	...
	for _, obHost := range componentData.AllowedOutboundHosts {
		outboundTable.AppendRow(table.Row{"Allowed", obHost})
	}
